...
if [[ -z $nameprefix || -z $dateprefix || -z $start_day || -z $end_day ]]; then
  echo -e "${red_bold}Usage: ./swift_size_check.sh <nameprefix> <dateprefix> <start_day> <end_day>${reset_color}"
  echo -e "${red_bold}Usage: ./swift_size_chek.sh <Name> <year-month> <1-31> <1-31>${reset_color}"
  exit 1
fi
